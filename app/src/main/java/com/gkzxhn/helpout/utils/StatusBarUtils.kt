...
object StatusBarUtils {
    ...
    fun immersive(window: Window, color: Int, @FloatRange(from = 0.0, to = 1.0) alpha: Float = 1f) {
        if (Build.VERSION.SDK_INT >= 21) {
            ...
            setTranslucentView(window.getDecorView() as ViewGroup, color, alpha)
    ...
    fun setMargin(context: Context, view: View) {
        if (Build.VERSION.SDK_INT >= MIN_API) {
            ...
            if (lp is ViewGroup.MarginLayoutParams) {
                (lp as ViewGroup.MarginLayoutParams).topMargin += getStatusBarHeight(context)//增高
            }
    ...
    fun setTranslucentView(container: ViewGroup, color: Int, @FloatRange(from = 0.0, to = 1.0) alpha: Float) {
        if (Build.VERSION.SDK_INT >= 19) {
            ...
            var translucentView: View? = container.findViewById<View>(android.R.id.custom)
            if (translucentView == null && mixtureColor != 0) {
                translucentView = View(container.getContext())
    ...
    fun getStatusBarHeight(context: Context): Int {
        ...
        if (resId > 0) {
            ...
            result = TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP,
                    result.toFloat(), Resources.getSystem().getDisplayMetrics()).toInt()
        }
        return result
    }
}//<editor-fold desc="沉侵">
